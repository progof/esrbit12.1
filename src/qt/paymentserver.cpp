...
const int ELSPERO_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString ELSPERO_IPC_PREFIX("elspero:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(ELSPERO_IPC_PREFIX, Qt::CaseInsensitive)) // elspero: URI
        {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    Q_FOREACH (const QString& r, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(ELSPERO_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = NULL;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(ELSPERO_IPC_CONNECT_TIMEOUT);
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(ELSPERO_IPC_PREFIX, Qt::CaseInsensitive)) // elspero: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            {
                CElsperoAddress address(recipient.address.toStdString());
                if (!address.IsValid()) {
