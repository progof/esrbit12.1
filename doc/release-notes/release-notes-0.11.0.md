Elspero Core version 0.11.0 is now available from:

  <https://elspero.org/bin/elspero-core-0.11.0/>
...
backwards-compatible with pre-0.10 versions of Elspero Core or other software:
...
defaults to 0.00001. This will cause transactions with fewer ESR/kB fee to be
...
For example, add the following to `elspero.conf`:
...
related to the blockchain in the elspero system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Elspero Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Elspero Core will 
...
For an example script see [elspero-submittx](https://github.com/laanwj/elspero-submittx).
...
- #5334 `cf87536` libelsperoconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new elspero-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only ElsperoAmountField
- #5753 `0f44672` Add elspero logo to about screen
...
- #5809 `46bfbe7` Add elspero-cli man page
...
- #5849 `41259ca` contrib/init/elsperod.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Elspero-Qt.app to Elspero-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for elspero-tx)
